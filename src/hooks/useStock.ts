...
export function useStock() {
  ...
  const load = useCallback(async () => {
    try {
      const [a, b] = await Promise.all([
        (supabase.from("stock_items")).select("id, name, quantity, unit_value, notes").order("created_at", { ascending: false }),
        (supabase.from("stock_history")).select("id, stock_item_id, old_quantity, new_quantity, changed_at").order("changed_at", { ascending: false }),
  ...
  const updateItem = useCallback(async (id: string, updates: Partial<StockItem>) => {
    try {
      ...
      const { error } = await (supabase.from("stock_items")).update(payload).eq("id", id) as { error: { message: string } | null };
      ...
      if (current && updates.quantity !== undefined && updates.quantity !== current.quantity) {
        const { data: histRow } = await (supabase.from("stock_history"))
  ...
  const deleteItem = useCallback(async (id: string) => {
    try {
      const { error } = await (supabase.from("stock_items")).delete().eq("id", id) as { error: { message: string } | null };
